...
Class Actions extends DBConnection{
    ...
    function save_department(){
        ...
        if($check >0){
            $resp['status']="failed";
            $resp['msg'] = "Nombre de División Existe Actualmente";
        }else{
            @$save = $this->query($sql);
            if($save){
                $resp['status']="success";
                if(empty($id))
                    $resp['msg'] = "División Actualizada Exitósamente";
                else
                    $resp['msg'] = "División Actualizada Exitósamente";
            }else{
                $resp['status']="failed";
                if(empty($id))
                    $resp['msg'] = "No se pudo Guardar el Nuevo División";
                else
                    $resp['msg'] = "Proceso de Actualización de División Falló";
                    $resp['error']=$this->lastErrorMsg();
            }
        }
    ...
    function delete_department(){
        ...
        if($delete){
            $resp['status']='success';
            $_SESSION['flashdata']['type'] = 'success';
            $_SESSION['flashdata']['msg'] = 'División Eliminado Exitósamente';
        }else{
            $resp['status']='failed';
            $resp['error']=$this->lastErrorMsg();
        }
    ...
    function update_stat_dept(){
        ...
        if($update){
            $resp['status'] = 'success';
            $resp['msg'] = 'Estado de División Actualizado Exitósamente';
    ...
    function save_position(){
        ...
        if($check >0){
            $resp['status']="failed";
            $resp['msg'] = "Nombre de Grupo Existe Actualmente";
        }else{
            @$save = $this->query($sql);
            if($save){
                $resp['status']="success";
                if(empty($id))
                    $resp['msg'] = "Grupo Guardado Exitósamente";
                else
                    $resp['msg'] = "Grupo Actualizado Exitósamente";
            }else{
                $resp['status']="failed";
                if(empty($id))
                    $resp['msg'] = "Proceso de Nuevo Grupo Falló";
                else
                    $resp['msg'] = "Proceso de Actualización de Grupo";
                    $resp['error']=$this->lastErrorMsg();
            }
        }
    ...
    function delete_position(){
        ...
        if($delete){
            $resp['status']='success';
            $_SESSION['flashdata']['type'] = 'success';
            $_SESSION['flashdata']['msg'] = 'Grupo Eliminado Exitósamente';
        }else{
            $resp['status']='failed';
            $resp['error']=$this->lastErrorMsg();
        }
    ...
    function update_stat_position(){
        ...
        if($update){
            $resp['status'] = 'success';
            $resp['msg'] = 'Estado de Grupo Actualizada Exitósamente';
            $_SESSION['flashdata']['type'] = $resp['status'];
            $_SESSION['flashdata']['msg'] = $resp['msg'];
        }else{
            $resp['status'] = 'failed';
    ...
    function save_employee(){
        ...
        if($check >0){
            $resp['status']="failed";
            $resp['msg'] = "Código de Alumno Existe Actualmente";
        }else{
            @$save = $this->query($sql);
            if($save){
                $resp['status']="success";
                if(empty($id)){
                    $resp['msg'] = "Alumno Guardado Exitósamente";
                    $eid = $this->query("SELECT last_insert_rowid()")->fetchArray()[0];
                }else{
                    $resp['msg'] = "Alumno Actualizado Exitósamente";
                    $eid = $id;
               } 
               ...
               if(isset($_FILES['avatar']['tmp_name']) && !empty($_FILES['avatar']['tmp_name'])){
                ...
                $resp['status']="failed";
                if(empty($id))
                    $resp['msg'] = "Proceso de Generación de Nuevo Alumno Fallido";
                else
                    $resp['msg'] = "Proceso de Actualización de Alumno Fallido";
                    $resp['error']=$this->lastErrorMsg();
            }
        }
    ...
    function delete_employee(){
        ...
        if($delete){
            $resp['status']='success';
            $_SESSION['flashdata']['type'] = 'success';
            $_SESSION['flashdata']['msg'] = 'Alumno Eliminado Exitósamente';
